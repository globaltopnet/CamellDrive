...
class ActionSheetFallbackHelpers {
    ...
    static flattenMenuConfig(menuConfig) {
        ...
        const getCurrentParentItemsCount = () => (currentParent?.menuItems?.length ?? 0);
        const didReachEnd = () => (getCurrentIndexPath() >= (getCurrentParentItemsCount() - 1));
        ...
        while (true) {
            ...
            if (!isCurrentItemEmpty && ActionSheetFallbackHelpers.isObjectMenuConfig(currentItem)) {
                ...
                const isInlineSubmenu = currentItem.menuOptions?.includes('displayInline') ?? false;
                const inlineSubmenuItemsCount = currentItem.menuItems?.length ?? 0;
                if (isInlineSubmenu && inlineSubmenuItemsCount > 0) {
                    const inlineMenu = currentParent?.menuItems.splice(getCurrentIndexPath(), 1);
                    const inlineMenuItems = 
                    // @ts-ignore
                    inlineMenu?.[0]?.menuItems ?? [];
                    currentParent?.menuItems.splice(getCurrentIndexPath(), 0, ...inlineMenuItems);
                }
                else {
...
export class ActionSheetFallback {
    ...
    static async show(menuConfig) {
        ...
        const menuConfigCopy = JSON.parse(JSON.stringify(menuConfig));
        // handle 'inlineMenu' submenu's
        ActionSheetFallbackHelpers.flattenMenuConfig(menuConfigCopy);
        while (true) {
            const menuItem = ActionSheetFallbackHelpers.getItemFromMenuConfig(menuConfigCopy, indexPath);
