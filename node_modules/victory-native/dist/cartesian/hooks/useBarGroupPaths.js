        ...
        return points.map((pointSet, i) => {
            ...
            pointSet.forEach(({ x, y }) => {
                if (typeof y !== "number")
                    return;
                if (!roundedCorners) {
                    p.addRect(react_native_skia_1.Skia.XYWHRect(x + offset, y, barWidth, chartBounds.bottom - y));
                }
                else {
                    const roundedRectPath = react_native_skia_1.Skia.Path.MakeFromSVGString((0, createRoundedRectPath_1.createRoundedRectPath)(x + offset, y, barWidth, chartBounds.bottom - y, roundedCorners));
